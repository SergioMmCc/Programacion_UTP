...
void solver(struct graph *G, int source, int destination) {
    int dD[MAXV + 1], piD[MAXV + 1], dS[MAXV + 1], piS[MAXV + 1], idVertex, result = 0;
    dijkstra (G, dS, piS, source);
    dijkstra (G, dD, piD, destination);
