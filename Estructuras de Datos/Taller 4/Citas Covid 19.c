...
struct nodeRBTree *RB_Insert (struct nodeRBTree *T, int k, char saveName []) {
    struct nodeRBTree *z, *x, *y;
    z = (struct nodeRBTree *) malloc (sizeof (struct nodeRBTree));
    z->color = RED;
    z->key = k;
    strcpy (z->name, saveName);
    //z->frequency = 1;
...
int main () {
    int idLine = 1000000, savePriority;
    char saveName [21];
    struct nodeRBTree *T, *max;
    T = assignNilLeaf ();
    
    while (scanf ("%s", saveName) != EOF) {
        if (strcmp (saveName, "V") == 0) {
            if (T->key == NILKey)
                printf ("\n");
            else {
                max = treeMaximum (T);
            ...
            scanf ("%d", &savePriority);
            savePriority = savePriority * 1000000 + idLine;
            T = RB_Insert (T, savePriority, saveName);
            idLine--;
        }
    }
