...
int main() {
    int operation, element, totalCases, listLenght, addElement, totalQueries;
    int valuePredecessor, valueSucessor;
    struct nodeRBTree *T, *z, *predecessor, *sucessor;
    T = assignNilLeaf ();
    ...
    while (totalCases--) {
        ...
        while (listLenght--) {
            scanf ("%d", &addElement);
            z = treeSearch (T, addElement);
            if (z->key == NILKey)
                T = RB_Insert (T, addElement);
            else
                z->frequency++;  
        }
        T = RB_Insert (T, -1);
        ...
        while (totalQueries--) {
            ...
            if (operation == 1) {
                ...
                if (z->key != NILKey) {
                    predecessor = treePredecessor (z);
                    valuePredecessor = predecessor->key;
                    sucessor = treeSucessor (z);
                    valueSucessor = sucessor->key;
                }
                else {
                ...
                printf ("%d %d\n", valuePredecessor, valueSucessor);
            }
            else if (operation == 2) {
                z = treeSearch (T, element);
                if (z->key == NILKey)
