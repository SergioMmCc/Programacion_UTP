...
struct nodeRBTree *RB_Insert (struct nodeRBTree *T, int k, int arrival, char name[]) {
...
int main () {
    int transacciones, operation, idLine, arrival, priority, triage, doctorTime;
    char saveName[21];
    struct nodeRBTree *z, *T;
    T = assignNilLeaf ();
    
    while (scanf ("%d", &transacciones) != EOF) {
        idLine = 200000;
        while (transacciones--) {
            scanf("%d", &operation);
            if (operation == 1) {
                scanf ("%d %d %s", &arrival, &triage, saveName);
                priority = triage * 200000 - idLine;
                
                T = RB_Insert (T, priority, arrival, saveName);
                idLine--;
            }
            else if (operation == 2) { 
                scanf ("%d", &doctorTime);
                z = treeMinimum (T);
                printf ("%d %d %d %s\n", z->arrival, doctorTime, doctorTime - z->arrival, z->name);
                T = RB_Delete (T, z);
            }
        }
