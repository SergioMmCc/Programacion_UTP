...
int main () {
    int i, iIndex, j, jIndex, k, kIndex, addNumber;
    struct nodeRBTree *z, *T;
    
    T = assignNilLeaf ();
    
    for (iIndex = 0, i = 1; iIndex <= MAXN; iIndex += i, i++) {
        for (jIndex = iIndex, j = i; jIndex <= MAXN; jIndex += j, j++) {
            for (kIndex = jIndex, k = j; kIndex <= MAXN; kIndex += k, k++) {
                addNumber = kIndex + jIndex + iIndex;
                if (addNumber <= MAXN) {
                    z = treeSearch (T, addNumber);
                    if (z->key != NILKey)
                        z->freq++;
                    else
                        T = RB_Insert (T, addNumber);
                }
                else
                    break;
            }
    ...
    int cases, lookForNumber;
    scanf ("%d", &cases);
    
    while (cases--) {
        scanf ("%d", &lookForNumber);
        z = treeSearch (T, lookForNumber);
        printf ("%d\n", z->freq);        
    }
